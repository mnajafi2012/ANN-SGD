...
for iter= 1: iters
    rng(iter); % to emphasize the difference in weight initializations
    ...
    for i = 1: epoch
        ...
        for n = 1 : N_train
            ...
            for l = 1 : L
                ...
                %% 11. Update weights (Stochastic method)
...
for l = L:-1:1
    if (l == L)
        ...
        d_E_EBP{2} = tmp; %the accumulative error for the hidden layer
    else
        tmp = (delta(:, l) * Z_in')'; % Weights' errors hidden layer
        d_E_EBP{1} = tmp; % the accumulative error for the output layer
    end
