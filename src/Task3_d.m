function Task3_d
% MP2 Task 3. (d)
% run this code by simply typing Task3_d in the workspace.
...
for iter= 1: iters
    ...
    misclassified_percentage = report(Y, t_test, N_test);
end


end

function percentage = report(Y, t_test, N)
counter = 0;
    for i = 1: N
       if (Y(i) <= 0.5 ) Y(i) = 0; else Y(i) = 1; end
       % check whether the sample is misclassified
       if (Y(i) ~= t_test(i)) counter = counter + 1; end
    end
