function Task3_e
% MP2 Task 3. (e)
% run this code by simply typing Task3_e in the workspace.

% CE: E = − [t ln(y) + (1 − t) ln(1 − y)]
% CE's gradient: ∂E/∂y = (y - t)/y(1 - y)

% The assumption is that we have solely 1 hidden layer including different
...
% do the following steps for different size of the hidden layer from 3:10
for h = 8: H
    seed = h;
    ...
    while ~stop
        ...
        for n = 1 : N_train
            ...
            for l = 1 : L
                ...
                [alpha, y, Z_out] = FP(D_train(:, n), w, L);
                Y = [Y; y]; % MLP's output
    ...
    %% 12. test
    Y = test(w, D_val, N_val, L);
    
    %% 13. plot
    p = [D_val(1,:); D_val(2,:); Y(:)'];
    plot3(p(1,:), p(2,:), p(3,:),'o');
    title ('The classified validation set into two classes of 0 and 1');
    xlabel ('x_1'); ylabel('x_2'); zlabel('MLP output');
    %legend('Validation sample');
    
    %% 14. report the misclassified samples' percentage
    misclassified_percentage = report(Y, t_val, N_val);
    disp('num of neurons in the hidden layer: ');
    disp(h);
    disp('The misclassified percentage: ');
    disp(misclassified_percentage);
end


end

function percentage = report(Y, t_val, N)
counter = 0;
    for i = 1: N
       if (Y(i) <= 0.5 ) Y(i) = 0; else Y(i) = 1; end
       % check whether the sample is misclassified
       if (Y(i) ~= t_val(i)) counter = counter + 1; end
    end
...
function [alpha, y, Z_out] = FP(sample, w_0, L)
